...
async function claimLeadRoyalty({ lead, claimant, financialYear }) {
  ...
  const approval = await PendingApproval.findOneAndUpdate(
    ...
    { $set: {
      ...
      payload: { claimKey, leadId, company, financialYear, claimantId, requestedById: claimantId, claimantName, claimantEmail: claimant?.email || '', originalCreator, originalCreatorId, originalCreatorEmail },
  ...
  const description = `${claimantName} claimed royalty for ${company}${financialYear ? ` (${financialYear})` : ''}. Original lead creator: ${originalCreator}. Please review and take action.`;
