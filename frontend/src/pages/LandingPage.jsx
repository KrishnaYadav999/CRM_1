...
export default function LandingPage() {
  ...
  return <div ref={rootRef} className="landing-page">
    <header className="lp-nav">
      <div className="lp-nav-inner"><Link to="/" className="lp-brand-link" aria-label="Anant Tattva home"><BrandMark showCopy={false} /></Link><nav className={menuOpen ? 'is-open' : ''} aria-label="Mobile navigation"><Link className="lp-mobile-login" to="/login">Login <ArrowRight /></Link></nav><div className="lp-nav-actions"><a className="lp-contact" href="mailto:info@ananttattva.com"><small>Talk to us</small><b>info@ananttattva.com</b></a><Link className="lp-login-link" to="/login"><UsersRound /> Login</Link><a className="lp-primary-button small" href="#contact">Get started</a><button className="lp-menu-button" onClick={() => setMenuOpen((open) => !open)} aria-label="Toggle menu">{menuOpen ? <X /> : <Menu />}</button></div></div>
    ...
    <footer className="lp-footer"><div className="lp-container"><BrandMark/><p>Smarter operations for a more responsible future.</p><div className="lp-footer-addresses"><address><b>Mumbai Office</b><span>1st Floor, A/25, Technocraft House, Road No. 3, MIDC, Andheri East, Mumbai - 400093</span></address><address><b>Ahmedabad Office</b><span>World Trade Tower (WTT), Office No. 1112, A Block, 11th Floor, Makarba, Ahmedabad – 380051</span></address></div><div className="lp-footer-links"><a href="#solutions">Solutions</a><a href="#customers">Customers</a><a href="mailto:info@ananttattva.com">Contact</a><Link to="/login">CRM Login</Link></div><small>© {new Date().getFullYear()} Anant Tattva Private Limited. All rights reserved.</small></div></footer>
  </div>
}
