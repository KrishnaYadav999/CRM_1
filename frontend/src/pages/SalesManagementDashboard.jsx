...
export default function SalesManagementDashboard() {
  ...
  const carryForwardChart = useMemo(() => (data.monthlyCarryForward || [])
    .filter((row) => row.totalAvailable || row.closedThisMonth || row.closingPending)
    .slice(-12), [data.monthlyCarryForward])
    ...
    <div className="mx-auto w-full max-w-[1600px] p-4 sm:p-6 lg:p-8">
      ...
      {loading ? <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-40 animate-pulse rounded-3xl bg-white" />)}</div> : <>
        ...
        <section className="mt-6 hidden overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm sm:block">
          ...
          <div className="border-b border-slate-100 p-5"><div className="h-[340px]"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={carryForwardChart} margin={{ top: 12, right: 18, left: -5, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} /><XAxis dataKey="month" tickFormatter={monthLabel} tick={{ fontSize: 11, fontWeight: 700 }} /><YAxis allowDecimals={false} tick={{ fontSize: 11 }} /><Tooltip content={<ChartTooltip />} /><Legend wrapperStyle={{ fontSize: 11, fontWeight: 700 }} /><Bar dataKey="openingPending" name="Previous Pending" stackId="available" fill="#F59E0B" /><Bar dataKey="newLeads" name="New Leads" stackId="available" fill="#0EA5E9" radius={[4, 4, 0, 0]} /><Line type="monotone" dataKey="closingPending" name="Closing Pending" stroke="#EF4444" strokeWidth={3} dot={{ r: 4, fill: '#EF4444' }} /></ComposedChart></ResponsiveContainer></div><p className="mt-2 text-center text-[11px] font-bold text-slate-500">Orange and blue form total availability. The red line is the pending balance carried into the next month.</p></div>
