...
function QuotationMis({ quotations }) {
  ...
  return <DashboardShell currentUser={currentUser}>
    <div className="mx-auto w-full max-w-[1600px] p-4 sm:p-6 lg:p-8">
      ...
      {loading ? <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-40 animate-pulse rounded-3xl bg-white" />)}</div> : <>
        ...
        <section className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm max-sm:hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><div><p className="text-[10px] font-black uppercase tracking-[.18em] text-emerald-700">Section 2</p><h2 className="mt-1 text-xl font-black text-slate-950">Lead Owner Sales Performance</h2><p className="mt-1 text-xs font-bold text-slate-500">PO value stays with the permanent lead owner. Reporting manager is shown separately.</p></div><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600">{managers.length} owners</span></div>
        ...
        {activeTab === 'quotations' && <QuotationMis quotations={filteredQuotations} />}
