    ...
    <div className="mx-auto w-full max-w-[1600px] p-4 sm:p-6 lg:p-8">
      ...
      {loading ? <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }).map((_, index) => <div key={index} className="h-40 animate-pulse rounded-3xl bg-white" />)}</div> : <>
        ...
        <section className="mt-6 hidden overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm sm:block">
          <div className="border-b border-slate-100 px-5 py-4"><p className="text-[10px] font-black uppercase tracking-[.18em] text-emerald-700">Monthly movement</p><h2 className="mt-1 text-xl font-black text-slate-950">Lead Carry-Forward</h2><p className="mt-1 text-xs font-bold text-slate-500">Opening pending is kept separate from new leads. Closing pending becomes the next month's opening balance.</p></div>
