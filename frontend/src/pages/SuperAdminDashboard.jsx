        ...
        {misPage && <section className="mt-4 overflow-hidden rounded-2xl border border-cyan-200 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-slate-50 text-left text-[10px] font-black uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">Team / User</th><th className="px-5 py-3">Level</th><th className="px-5 py-3">Reports To</th><th className="px-5 py-3 text-right">Client Masters</th><th className="px-5 py-3 text-right">Total Draft</th><th className="px-5 py-3 text-right">Total Submitted</th><th className="px-5 py-3">Completion</th></tr></thead><tbody>
            {operationGroups.flatMap((group) => {
              ...
              const teamRow = <tr key={`team-${group.id}`} className="border-t-2 border-cyan-100 bg-cyan-50/70 font-black text-slate-900"><td className="px-5 py-4"><span className="inline-flex items-center gap-2"><Building2 className="h-4 w-4 text-cyan-700" />{group.name}</span></td><td className="px-5 py-4"><span className="rounded-full bg-cyan-100 px-2.5 py-1 text-[10px] uppercase text-cyan-800">Team Total</span></td><td className="px-5 py-4">{group.manager?.name || '-'}</td><td className="px-5 py-4 text-right text-lg">{group.clientMasters}</td><td className="px-5 py-4 text-right text-amber-700">{group.draftClients || 0}</td><td className="px-5 py-4 text-right text-emerald-700">{group.submittedClients || 0}</td><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="h-2 w-32 overflow-hidden rounded-full bg-white ring-1 ring-cyan-100"><div className="h-full rounded-full bg-cyan-600" style={{ width: `${group.percentage}%` }} /></div><strong>{group.percentage}%</strong></div></td></tr>
              const peopleRows = people.map((row) => <tr key={`${group.id}-${row.id}`} className="border-t border-slate-100 font-semibold text-slate-700 hover:bg-slate-50"><td className="py-3 pl-10 pr-5"><button type="button" onClick={() => setWorkReportUser({ ...row, initialView: "clients" })} className="block text-left font-black text-slate-950 hover:text-cyan-700 hover:underline">{row.name}</button><small className="text-slate-500">{row.email}</small></td><td className="px-5 py-3"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${row === group.manager ? 'bg-violet-100 text-violet-700' : 'bg-emerald-100 text-emerald-700'}`}>{row === group.manager ? 'Manager' : 'User'}</span></td><td className="px-5 py-3">{row === group.manager ? '—' : group.manager?.name || 'Not assigned'}</td><td className="px-5 py-3 text-right font-black">{row.clientMasters || 0}</td><td className="px-5 py-3 text-right font-black text-amber-700">{row.draftClients || 0}</td><td className="px-5 py-3 text-right font-black text-emerald-700">{row.submittedClients || 0}</td><td className="px-5 py-3"><div className="flex items-center gap-3"><div className="h-2 w-32 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-emerald-600" style={{ width: `${row.clientCompletionPercentage || 0}%` }} /></div><strong>{row.clientCompletionPercentage || 0}%</strong></div></td></tr>)
