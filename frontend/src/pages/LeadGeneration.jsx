...
function TemporaryLeadFollowUpTracker({ row, onClose, onSaved }) {
  ...
  const Item = ({ item, previousItem = false }) => <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-2"><strong className="text-sm font-black text-slate-900">{item.scheduledDate || '-'}{item.scheduledTime ? ` at ${item.scheduledTime}` : ''}</strong><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase ${previousItem ? 'bg-slate-100 text-slate-500' : 'bg-orange-100 text-orange-700'}`}>{previousItem ? item.status || 'Previous' : 'Upcoming'}</span></div><div className="mt-2 flex gap-2"><span className="rounded-full bg-violet-50 px-2.5 py-1 text-[10px] font-black text-violet-700">{item.priority || 'Medium'} Priority</span></div><p className="mt-3 whitespace-pre-wrap text-sm font-semibold leading-6 text-slate-600">{item.remarks || 'No remarks added.'}</p></article>;
...
function QuotationClosureSummary({ quotation, items = [], poRows = [], setClosureDialog, uploadClosureFile }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-teal-200 bg-white shadow-sm"><header className="flex flex-wrap items-center justify-between gap-2 bg-teal-50 px-5 py-4"><div><p className="text-xs font-black uppercase tracking-wider text-teal-700">Quotation details auto-fetched</p><strong className="text-slate-900">{quotation.quotationNumber || 'Latest quotation'}</strong></div><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-teal-700">{combined ? 'Combined Price' : 'Individual Price'}</span></header><div className="overflow-auto"><table className="w-full min-w-[2050px] text-left text-xs"><thead className="bg-slate-100 uppercase text-slate-500"><tr>{['#', 'EPR / Service Period', 'Industry Type', 'Business Category', 'Service Category', 'Service Start Date', 'Service End Date', 'Applicant Type', 'Unit', 'UOM', 'Basic Amount (INR)', 'PO Number', 'PO Amount (INR)', 'PO Proof', 'Service'].map((heading) => <th key={heading} className="whitespace-nowrap p-3">{heading}</th>)}</tr></thead><tbody>{rows.map((item, index) => { const po = poRows[index] || {}; return <tr key={item.id || index} className="border-t align-middle"><td className="p-3 font-black">{index + 1}</td><td className="p-3 font-black text-slate-700">{po.fy || item.servicesForYear || item.financialYear || '-'}</td><td className="p-3">{item.industryType || '-'}</td><td className="p-3">{item.businessCategory || '-'}</td><td className="p-3">{item.serviceCategory || item.eprCategory || '-'}</td><td className="p-3">{item.serviceStartDate || '-'}</td><td className="p-3">{item.serviceEndDate || '-'}</td><td className="p-3">{item.subApplicantType || item.piboCategory || item.applicantType || '-'}</td><td className="p-3">{item.unit || '1'}</td><td className="p-3">{item.unitLabel || '-'}</td>{(!combined || index === 0) && <td rowSpan={combined ? rows.length : 1} className={`p-3 font-black text-emerald-700 ${combined ? 'border-l border-r bg-emerald-50 align-middle' : ''}`}>₹{Number(combined ? quotation.combinedBasicAmount || quotation.grandTotal : (Number(item.unit) || 1) * (Number(item.basicAmount) || 0)).toLocaleString('en-IN')}</td>}<td className="p-3"><input className="form-input min-w-36" value={po.poNumber || ''} onChange={(event) => updatePo(index, { poNumber: event.target.value })} placeholder="PO Number" /></td><td className="p-3"><div className="relative min-w-40"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 font-black text-slate-500">₹</span><input type="number" min="0.01" step="0.01" className="form-input pl-7" value={po.poAmount ?? ''} onChange={(event) => updatePo(index, { poAmount: event.target.value })} placeholder="PO Amount" /></div></td><td className="p-3"><label className="flex min-h-11 min-w-36 cursor-pointer items-center justify-center rounded-xl border border-emerald-300 bg-emerald-50 px-3 font-black text-emerald-700"><Upload className="mr-2 h-4 w-4" />{po.poFileName || 'Choose File'}<input type="file" className="sr-only" accept="image/*,.pdf" onChange={(event) => uploadClosureFile(event, 'po', index)} /></label></td><td className="p-3 font-bold">{po.services?.[0] || item.servicesOffered || item.applicableService || '-'}</td></tr>; })}{!rows.length && <tr><td colSpan="15" className="p-8 text-center font-bold text-slate-400">No quotation service rows available.</td></tr>}</tbody></table></div></section>;
}
