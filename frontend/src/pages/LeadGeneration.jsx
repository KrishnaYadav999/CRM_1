...
export default function LeadGeneration() {
  ...
  function requestLeadClosure(index, value) {
    ...
    const fetchedPoRows = quotationItems.length ? quotationItems.map(quoteRow) : [quoteRow()];
    ...
    setClosureDialog({ index, value, reviewMode, choice: reviewMode ? 'yes' : '', quotation: latestQuotation, quotationItems, poModeConfirmed: true, poMode: 'quotation', poYearRows, createPoRow: () => quoteRow({}, poYearRows.length), approvalProofUrl: '', approvalProofName: '' });
  }
        ...
        <div className="fixed inset-0 z-[125] bg-slate-50" role="dialog" aria-modal="true">
          <section className="flex h-screen w-full flex-col overflow-hidden bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-6 lg:px-10">
              ...
              {closureDialog.choice === 'yes' && <div className="mt-6"><QuotationClosureSummary quotation={closureDialog.quotation} items={closureDialog.quotationItems} poRows={closureDialog.poYearRows} setClosureDialog={setClosureDialog} uploadClosureFile={uploadClosureFile} annualReturnYearOptions={annualReturnYearOptions} /><div className="mt-4 flex items-center justify-between"><p className="text-sm font-black text-slate-600">{closureDialog.poYearRows.length} quotation service row{closureDialog.poYearRows.length === 1 ? '' : 's'} ready for PO details</p><button type="button" onClick={() => setClosureDialog((current) => ({ ...current, poYearRows: [...current.poYearRows, current.createPoRow()] }))} className="rounded-xl border border-emerald-300 bg-emerald-50 px-4 py-3 font-black text-emerald-700"><Plus className="mr-1 inline h-4 w-4" />Add Extra PO</button></div></div>}
...
function QuotationClosureSummary({ quotation, items = [], poRows = [], setClosureDialog, uploadClosureFile, annualReturnYearOptions = [] }) {
  ...
  const updateCombinedAmount = (value) => setClosureDialog((current) => ({ ...current, poYearRows: current.poYearRows.map((row) => ({ ...row, poAmount: value })) }));
  return <section className="overflow-hidden rounded-2xl border border-teal-200 bg-white shadow-sm"><header className="flex flex-wrap items-center justify-between gap-2 bg-teal-50 px-5 py-4"><div><p className="text-xs font-black uppercase tracking-wider text-teal-700">Quotation details auto-fetched</p><strong className="text-slate-900">{quotation.quotationNumber || 'Latest quotation'}</strong></div><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-teal-700">{combined ? 'Combined Price' : 'Individual Price'}</span></header><div className="overflow-auto"><table className="w-full min-w-[1900px] text-left text-xs"><thead className="bg-slate-100 uppercase text-slate-500"><tr>{['#', 'EPR / Service Period', 'Industry Type', 'Business Category', 'Service Category', 'Service Start Date', 'Service End Date', 'Applicant Type', 'Unit', 'UOM', 'Basic Amount (INR)', 'PO Number', 'PO Proof', 'Service'].map((heading) => <th key={heading} className="whitespace-nowrap p-3">{heading}</th>)}</tr></thead><tbody>{rows.map((item, index) => { const po = poRows[index] || {}; return <tr key={item.id || index} className="border-t align-middle"><td className="p-3 font-black">{index + 1}</td><td className="p-3"><select className="form-input min-w-28" value={po.fy || ''} onChange={(event) => updatePo(index, { fy: event.target.value })}><option value="">FY Year</option>{annualReturnYearOptions.map((fy) => <option key={fy}>{fy}</option>)}</select></td><td className="p-3">{item.industryType || '-'}</td><td className="p-3">{item.businessCategory || '-'}</td><td className="p-3">{item.serviceCategory || item.eprCategory || '-'}</td><td className="p-3">{item.serviceStartDate || '-'}</td><td className="p-3">{item.serviceEndDate || '-'}</td><td className="p-3">{item.subApplicantType || item.piboCategory || item.applicantType || '-'}</td><td className="p-3">{item.unit || '1'}</td><td className="p-3">{item.unitLabel || '-'}</td>{(!combined || index === 0) && <td rowSpan={combined ? rows.length : 1} className={`p-3 ${combined ? 'border-l border-r bg-emerald-50 align-middle' : ''}`}><label className="block min-w-36"><span className="mb-1 block text-[10px] font-black uppercase text-emerald-700">{combined ? 'Combined Basic Amount' : 'Basic Amount'}</span><input className="form-input font-black text-emerald-700" type="number" min="0" value={po.poAmount || ''} onChange={(event) => combined ? updateCombinedAmount(event.target.value) : updatePo(index, { poAmount: event.target.value })} /></label></td>}<td className="p-3"><input className="form-input min-w-36" value={po.poNumber || ''} onChange={(event) => updatePo(index, { poNumber: event.target.value })} placeholder="PO Number" /></td><td className="p-3"><label className="flex min-h-11 min-w-36 cursor-pointer items-center justify-center rounded-xl border border-emerald-300 bg-emerald-50 px-3 font-black text-emerald-700"><Upload className="mr-2 h-4 w-4" />{po.poFileName || 'Choose File'}<input type="file" className="sr-only" accept="image/*,.pdf" onChange={(event) => uploadClosureFile(event, 'po', index)} /></label></td><td className="p-3 font-bold">{po.services?.[0] || item.servicesOffered || item.applicableService || '-'}</td></tr>; })}{!rows.length && <tr><td colSpan="14" className="p-8 text-center font-bold text-slate-400">No quotation service rows available.</td></tr>}</tbody></table></div></section>;
}
