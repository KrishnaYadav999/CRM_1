...
export default function LeadGeneration() {
  ...
  function requestLeadClosure(index, value) {
    ...
    const quotationItems = latestQuotation?.items || [];
    const defaultAmount = latestQuotation?.pricingMode === 'combined'
      ...
      : quotationItems.reduce((sum, item) => sum + ((Number(item.unit) || 1) * (Number(item.basicAmount) || 0)), 0);
    const quoteRow = (fy = matchingService.firstAnnualReturnYearApplicable || '') => ({
      fy, poNumber: '', poAmount: defaultAmount, poFileUrl: '', poFileName: '',
      services: matchingService.servicesOffered ? [matchingService.servicesOffered] : [],
      quotationId: latestQuotation?._id || latestQuotation?.id || '', quotationNumber: latestQuotation?.quotationNumber || '',
      quotationItems, quotationCreatedById: latestQuotation?.createdBy?._id || latestQuotation?.createdBy || '',
      quotationCreatedByEmail: latestQuotation?.createdBy?.email || latestQuotation?.createdByEmail || ''
    });
    setClosureDialog({ index, value, reviewMode, choice: reviewMode ? 'yes' : '', quotation: latestQuotation, quotationItems, poModeConfirmed: false, poMode: '', poYearRows: matchingAssignment.poYearRows?.length ? matchingAssignment.poYearRows : [quoteRow()], createPoRow: quoteRow, approvalProofUrl: '', approvalProofName: '' });
  }
        ...
        <div className="fixed inset-0 z-[125] grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
          <section className="max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div className="max-h-[calc(92vh-170px)] overflow-y-auto p-6">
              {!closureDialog.reviewMode && <div className="grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => setClosureDialog((current) => ({ ...current, choice: 'yes', poModeConfirmed: false, poMode: '' }))} className={`rounded-2xl border-2 p-5 text-left ${closureDialog.choice === 'yes' ? 'border-emerald-500 bg-emerald-50 text-emerald-800' : 'border-slate-200'}`}><strong className="text-lg font-black">Yes — PO Received</strong><span className="mt-1 block text-sm font-bold">Enter all PO details before closure.</span></button><button type="button" onClick={() => setClosureDialog((current) => ({ ...current, choice: 'no' }))} className={`rounded-2xl border-2 p-5 text-left ${closureDialog.choice === 'no' ? 'border-amber-500 bg-amber-50 text-amber-800' : 'border-slate-200'}`}><strong className="text-lg font-black">No — Close with Approval</strong><span className="mt-1 block text-sm font-bold">Upload Super Admin email/message approval proof.</span></button></div>}
              ...
              {closureDialog.choice === 'yes' && !closureDialog.poModeConfirmed && <div className="mt-6 rounded-2xl border border-blue-200 bg-blue-50 p-5"><h3 className="text-lg font-black text-slate-900">How many Purchase Orders have you received?</h3><p className="mt-1 text-sm font-semibold text-slate-600">Select one PO or multiple POs. Quotation values will be used as the starting details.</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><button type="button" onClick={() => setClosureDialog((current) => ({ ...current, poMode: 'single', poModeConfirmed: true, poYearRows: current.poYearRows.slice(0, 1) }))} className="rounded-xl bg-emerald-700 px-5 py-4 font-black text-white">One Purchase Order</button><button type="button" onClick={() => setClosureDialog((current) => ({ ...current, poMode: 'multiple', poModeConfirmed: true }))} className="rounded-xl border border-emerald-300 bg-white px-5 py-4 font-black text-emerald-800">Multiple Purchase Orders</button></div></div>}
              {closureDialog.choice === 'yes' && closureDialog.poModeConfirmed && <div className="mt-6"><QuotationClosureSummary quotation={closureDialog.quotation} items={closureDialog.quotationItems} /><div className="mt-5 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-slate-500">PO received for no. of years</p><strong className="text-2xl">{closureDialog.poYearRows.length}</strong></div>{closureDialog.poMode === 'multiple' && <button type="button" onClick={() => setClosureDialog((current) => ({ ...current, poYearRows: [...current.poYearRows, current.createPoRow('')] }))} className="rounded-xl bg-emerald-700 px-4 py-3 font-black text-white"><Plus className="mr-1 inline h-4 w-4" />Add Next PO</button>}</div><div className="mt-4 grid gap-3">{closureDialog.poYearRows.map((po, rowIndex) => <div key={rowIndex} className="grid items-center gap-3 rounded-2xl border bg-slate-50 p-4 md:grid-cols-[48px_130px_1fr_160px_170px_1fr_40px]"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white font-black">{rowIndex + 1}</span><select className="form-input" value={po.fy} onChange={(event) => setClosureDialog((current) => ({ ...current, poYearRows: current.poYearRows.map((row, index) => index === rowIndex ? { ...row, fy: event.target.value } : row) }))}><option value="">FY Year</option>{annualReturnYearOptions.map((fy) => <option key={fy}>{fy}</option>)}</select><input className="form-input" value={po.poNumber} onChange={(event) => setClosureDialog((current) => ({ ...current, poYearRows: current.poYearRows.map((row, index) => index === rowIndex ? { ...row, poNumber: event.target.value } : row) }))} placeholder="PO Number" /><input className="form-input" type="number" min="0" value={po.poAmount || ''} onChange={(event) => setClosureDialog((current) => ({ ...current, poYearRows: current.poYearRows.map((row, index) => index === rowIndex ? { ...row, poAmount: event.target.value } : row) }))} placeholder="PO Amount (INR)" /><label className="flex min-h-12 cursor-pointer items-center justify-center rounded-xl border border-emerald-300 bg-emerald-50 px-3 text-xs font-black text-emerald-700"><Upload className="mr-2 h-4 w-4" />{po.poFileName || 'Choose File'}<input type="file" className="sr-only" accept="image/*,.pdf" onChange={(event) => uploadClosureFile(event, 'po', rowIndex)} /></label><select className="form-input" value={po.services[0] || ''} onChange={(event) => setClosureDialog((current) => ({ ...current, poYearRows: current.poYearRows.map((row, index) => index === rowIndex ? { ...row, services: event.target.value ? [event.target.value] : [] } : row) }))}><option value="">Select Services</option>{[...new Set(serviceRows.map((row) => row.servicesOffered).filter(Boolean))].map((service) => <option key={service}>{service}</option>)}</select><button type="button" disabled={closureDialog.poYearRows.length === 1} onClick={() => setClosureDialog((current) => ({ ...current, poYearRows: current.poYearRows.filter((_, index) => index !== rowIndex) }))} className="grid h-10 w-10 place-items-center rounded-xl text-red-500 disabled:opacity-30"><X className="h-4 w-4" /></button></div>)}</div><div className="mt-4 flex justify-end"><button type="button" onClick={confirmLeadClosure} className="rounded-xl bg-blue-600 px-6 py-3 font-black text-white">Save PO Details</button></div></div>}
...
function QuotationClosureSummary({ quotation, items = [] }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-teal-200 bg-white"><header className="flex flex-wrap items-center justify-between gap-2 bg-teal-50 px-4 py-3"><div><p className="text-xs font-black uppercase tracking-wider text-teal-700">Quotation details auto-fetched</p><strong className="text-slate-900">{quotation.quotationNumber || 'Latest quotation'}</strong></div><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-teal-700">{quotation.pricingMode === 'combined' ? 'Combined Price' : 'Individual Price'}</span></header><div className="overflow-auto"><table className="w-full min-w-[1450px] text-left text-xs"><thead className="bg-slate-50 uppercase text-slate-500"><tr>{['EPR / Service Period', 'Industry Type', 'Business Category', 'Service Category', 'Service Start Date', 'Service End Date', 'Applicant Type', 'Unit', 'UOM', 'Basic Amount (INR)'].map((heading) => <th key={heading} className="p-3">{heading}</th>)}</tr></thead><tbody>{items.map((item, index) => <tr key={item.id || index} className="border-t"><td className="p-3 font-black">{item.servicesForYear || item.financialYear || '-'}</td><td className="p-3">{item.industryType || '-'}</td><td className="p-3">{item.businessCategory || '-'}</td><td className="p-3">{item.serviceCategory || item.eprCategory || '-'}</td><td className="p-3">{item.serviceStartDate || '-'}</td><td className="p-3">{item.serviceEndDate || '-'}</td><td className="p-3">{item.subApplicantType || item.piboCategory || item.applicantType || '-'}</td><td className="p-3">{item.unit || '1'}</td><td className="p-3">{item.unitLabel || '-'}</td><td className="p-3 font-black text-emerald-700">₹{Number(quotation.pricingMode === 'combined' ? quotation.combinedBasicAmount : item.basicAmount || 0).toLocaleString('en-IN')}</td></tr>)}{!items.length && <tr><td colSpan="10" className="p-6 text-center font-bold text-slate-400">No quotation service rows available.</td></tr>}</tbody></table></div></section>;
}
