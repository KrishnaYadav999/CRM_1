...
const blankPoYearRow = { fy: '', annualReturnYear: '', quotationNo: '', compliancePoDate: '', compliancePoFile: '', serviceCategory: [], value: '' }
    ...
    <div className="relative flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-[28px] border border-white/40 bg-slate-100 shadow-[0_30px_100px_rgba(15,23,42,.35)]">
      ...
      <div className="overflow-y-auto p-3 sm:p-6">
        <article className="mx-auto min-h-[900px] w-full max-w-[850px] bg-white px-7 py-7 text-[11px] leading-relaxed text-slate-950 shadow-xl sm:px-10 sm:py-9">
          ...
          <section className="mt-6"><p className="font-black">Scope of Work:</p><ol className="mt-2 list-decimal space-y-1 pl-5">{(row.scopeOfWork || []).filter(Boolean).length ? row.scopeOfWork.filter(Boolean).map((item, index) => <li key={index}>{item}</li>) : <li>No scope of work added.</li>}</ol></section>
...
export default function ProformaInvoices() {
  ...
  async function save() {
    ...
    try {
      const payload = { ...form, items: form.items.map((item) => ({ ...item, basicAmount: form.pricingMode === 'combined' ? 0 : item.basicAmount })), companyName: form.leadDetails.companyName, subtotal: total, grandTotal: total }
