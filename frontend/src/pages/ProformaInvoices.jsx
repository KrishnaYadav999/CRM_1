...
async function downloadProforma(row) {
  ...
  const columns = [
    { label: 'Business Category', x: page.left, width: 110 },
    { label: 'Service Category', x: page.left + 110, width: 128 },
    { label: 'EPR / Service Period', x: page.left + 238, width: 118 },
    { label: 'Services Offered', x: page.left + 356, width: 130 },
    { label: 'Unit', x: page.left + 486, width: 46 },
    { label: 'Basic Amount (INR)', x: page.left + 532, width: 95 }
  ]
  ...
  items.forEach((item, itemIndex) => {
    ...
    const values = [item.businessCategory || '-', item.eprCategory || item.serviceCategory || '-', eprOrServicePeriod(item), item.servicesOffered || '-', item.unit || '1', displayedAmount === '' ? '' : inr(displayedAmount)]
    ...
    columns.forEach((column, index) => {
      ...
      text(lines[index], column.x + (index === 5 ? column.width - 6 : 5), y + 12, { size: 6.5, bold: index === 1 || index === 5, align: index === 5 ? 'right' : undefined })
    })
    y += rowHeight
  })
  ;[
