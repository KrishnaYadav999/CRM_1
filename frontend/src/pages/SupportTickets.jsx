...
import { AlertCircle, ArrowRight, CheckCircle2, Clock3, FileQuestion, FileText, LifeBuoy, Loader2, MessageSquareText, Plus, RefreshCw, Search, Send, Sparkles, X } from 'lucide-react'
...
const emptyForm = { category: 'Lead', subject: '', description: '', referenceNumber: '', priority: 'Medium' }
...
export default function SupportTickets() {
  ...
  async function loadTickets() {
    ...
    } catch (err) { setError(err?.response?.data?.error || 'Tickets load nahi ho paaye.') }
  ...
  async function createTicket(event) {
    ...
    try {
      ...
      setNotice(`${data.ticket.ticketNumber} successfully raise ho gaya.`)
    } catch (err) { setError(err?.response?.data?.error || 'Ticket create nahi ho paaya.') }
  ...
  async function updateTicket(payload) {
    ...
    try {
      ...
      setTickets((items) => items.map((item) => item._id === data.ticket._id ? data.ticket : item)); setSelected(data.ticket); setReply(''); setPendingStatus(''); setStatusNote('')
    } catch (err) { setError(err?.response?.data?.error || 'Ticket update nahi ho paaya.') }
    finally { setSaving(false) }
  }
  ...
  return (
    <DashboardShell currentUser={currentUser}>
      ...
      {formOpen && <div className="fixed inset-0 z-[80] flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-sm"><form onSubmit={createTicket} className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-[28px] bg-white shadow-2xl"><div className="flex items-start justify-between border-b border-slate-100 p-6"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-emerald-700">New support request</p><h2 className="mt-2 text-2xl font-black text-slate-900">Raise a Ticket</h2><p className="mt-1 text-sm text-slate-500">Issue ko detail mein batayein, support team help karegi.</p></div><button type="button" onClick={() => setFormOpen(false)} className="rounded-xl bg-slate-100 p-2 text-slate-500 hover:bg-slate-200"><X className="h-5 w-5" /></button></div><div className="space-y-5 p-6"><div><label className="text-xs font-black uppercase tracking-wider text-slate-500">Select section *</label><div className="mt-2 grid gap-2 sm:grid-cols-3">{categories.map((c) => <button type="button" key={c.name} onClick={() => setForm((v) => ({ ...v, category: c.name }))} className={`rounded-xl border px-3 py-3 text-left text-xs font-black ${form.category === c.name ? 'border-emerald-500 bg-emerald-50 text-emerald-800 ring-2 ring-emerald-100' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}>{c.name}</button>)}</div></div><div className="grid gap-4 sm:grid-cols-2"><label className="text-xs font-black uppercase tracking-wider text-slate-500">Priority *<select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })} className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold normal-case tracking-normal text-slate-800 outline-none focus:border-emerald-500">{['Low','Medium','High','Urgent'].map((x) => <option key={x}>{x}</option>)}</select></label><label className="text-xs font-black uppercase tracking-wider text-slate-500">Reference number<input value={form.referenceNumber} onChange={(e) => setForm({ ...form, referenceNumber: e.target.value })} placeholder="Lead / Client / PI number" className="mt-2 h-12 w-full rounded-xl border border-slate-200 px-3 text-sm font-medium normal-case tracking-normal outline-none focus:border-emerald-500" /></label></div><label className="block text-xs font-black uppercase tracking-wider text-slate-500">Subject *<input required maxLength={160} value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="Short summary of your issue" className="mt-2 h-12 w-full rounded-xl border border-slate-200 px-4 text-sm font-medium normal-case tracking-normal outline-none focus:border-emerald-500" /></label><label className="block text-xs font-black uppercase tracking-wider text-slate-500">Describe your doubt / issue *<textarea required rows={6} maxLength={5000} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="What were you trying to do? What happened? Include any error message..." className="mt-2 w-full resize-none rounded-xl border border-slate-200 p-4 text-sm font-medium normal-case leading-6 tracking-normal outline-none focus:border-emerald-500" /></label></div><div className="flex flex-col-reverse gap-3 border-t border-slate-100 p-6 sm:flex-row sm:justify-end"><button type="button" onClick={() => setFormOpen(false)} className="h-12 rounded-xl border border-slate-200 px-5 text-sm font-black text-slate-600">Cancel</button><button disabled={saving} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-[#f45b0b] px-6 text-sm font-black text-white disabled:opacity-60">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Submit Ticket</button></div></form></div>}
      ...
      {pendingStatus && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm"><div className="w-full max-w-lg overflow-hidden rounded-[28px] bg-white shadow-2xl"><div className="bg-gradient-to-br from-emerald-700 to-emerald-600 p-6 text-white"><div className="flex items-start justify-between"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/15"><CheckCircle2 className="h-6 w-6" /></span><button onClick={() => { setPendingStatus(''); setStatusNote('') }} className="rounded-xl bg-white/10 p-2 hover:bg-white/20"><X className="h-5 w-5" /></button></div><h2 className="mt-5 text-2xl font-black">{pendingStatus === 'Closed' ? 'Close this ticket' : 'Resolve this ticket'}</h2><p className="mt-2 text-sm leading-6 text-emerald-50/90">Please explain what action you took. This note will be saved in the ticket conversation and emailed to the user.</p></div><div className="p-6"><label className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">{pendingStatus === 'Closed' ? 'Closure note' : 'Resolution note'} *</label><textarea autoFocus rows={6} maxLength={2000} value={statusNote} onChange={(e) => setStatusNote(e.target.value)} placeholder="Describe the issue found, action taken, and final outcome..." className="mt-3 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-50" /><div className="mt-2 flex justify-between text-[11px] text-slate-400"><span>This field is required.</span><span>{statusNote.length}/2000</span></div><div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button onClick={() => { setPendingStatus(''); setStatusNote('') }} className="h-12 rounded-xl border border-slate-200 px-5 text-sm font-black text-slate-600">Cancel</button><button disabled={saving || !statusNote.trim()} onClick={() => updateTicket({ status: pendingStatus, message: statusNote })} className="flex h-12 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-6 text-sm font-black text-white shadow-lg shadow-emerald-900/15 transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}Confirm {pendingStatus}</button></div></div></div></div>}
    </DashboardShell>
  )
}
