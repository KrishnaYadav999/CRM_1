...
export async function downloadOperationMisPdf({ groups, period }) {
  ...
  const body = groups.flatMap((group) => {
    ...
    return [[group.name, 'TEAM TOTAL', group.manager?.name || '-', group.clientMasters, group.submittedClients || 0, `${group.percentage}%`], ...people.map((row) => ['', row === group.manager ? 'Manager' : 'User', row.name, row.clientMasters || 0, row.submittedClients || 0, `${row.clientCompletionPercentage || 0}%`])]
  })
  autoTable(pdf, { startY: 38, margin: { left: 9, right: 9 }, head: [['Team', 'Level', 'User Name', 'Submitted Clients', 'Total Submitted', 'Completion']], body, theme: 'grid', headStyles: { fillColor: [8, 145, 178], fontStyle: 'bold' }, alternateRowStyles: { fillColor: [248, 250, 252] }, styles: { fontSize: 8, cellPadding: 2.5, lineColor: [203, 213, 225], lineWidth: 0.15 }, columnStyles: { 3: { halign: 'right' }, 4: { halign: 'right' }, 5: { halign: 'right' } }, didParseCell: (data) => { if (data.section === 'body' && data.row.raw?.[1] === 'TEAM TOTAL') { data.cell.styles.fillColor = [236, 254, 255]; data.cell.styles.fontStyle = 'bold' } } })
