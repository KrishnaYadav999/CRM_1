...
export default function SearchableSelect({ value = '', options = [], onChange, disabled = false, placeholder = 'Select or type to create new', allowCustom = true, canAddCustom = false, addLabel = 'option', onAddCustom, multiple = false }) {
  ...
  return (
    <>
      <div ref={triggerRef} className={`relative flex min-h-12 items-center rounded-xl border bg-white transition ${open ? 'border-emerald-500 ring-4 ring-emerald-100' : 'border-slate-200 hover:border-emerald-300'} ${disabled ? 'cursor-not-allowed bg-slate-100 opacity-70' : ''}`}>
        <input value={open ? query : (multiple ? selectedLabels.join(', ') : (selectedOption?.label || value))} disabled={disabled} onFocus={() => { setQuery(''); setOpen(true); }} onChange={(event) => { setQuery(event.target.value); if (!multiple && allowCustom && !canAddCustom) onChange(event.target.value); setOpen(true); }} placeholder={placeholder} className="min-w-0 flex-1 bg-transparent px-4 py-3 text-sm font-black text-slate-800 outline-none placeholder:text-slate-400" />
