...
function deriveAutoStages(client = {}, data = {}) {
  ...
  const closureDate = assignment.closedAt || service.closedAt || (allowLeadFallback ? (lead.closedAt || lead.closureDate) : '');
  ...
  return {
    ...
    poReceived: { completed: poReceived ? 'yes' : 'no', date: dateInputValue(firstDatedPo.poReceivedDate || firstDatedPo.poDate), details: [poNumbers.length ? `PO: ${poNumbers.join(', ')}` : '', poAmount ? `Value: ₹${poAmount.toLocaleString('en-IN')}` : '', assignment.poStatus ? `Status: ${assignment.poStatus}` : ''].filter(Boolean), proofs: poProofs }
  };
}
  ...
  return (
    <div className="fixed inset-0 z-[120] grid place-items-center bg-slate-950/55 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="mom-modal-title">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-white/70 bg-white shadow-2xl">
        <header className="flex items-start justify-between gap-4 bg-slate-900 p-6 text-white"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-orange-300">Minutes of meeting</p><h3 id="mom-modal-title" className="mt-1 text-2xl font-black">{milestoneLabel} MOM</h3></div><button type="button" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-xl bg-white/10 hover:bg-white/20" aria-label="Close MOM popup"><X className="h-5 w-5" /></button></header>
