    ...
    {activeTab === 'Purchase Data' && <div className="space-y-4 p-4">
      ...
      <div className="grid grid-cols-1 items-stretch gap-4 lg:grid-cols-2">
        {purchase.checklist.map((row, index) => {
          const proofRequired = row.yesNo === 'Yes';
          const proofMissing = proofRequired && !row.files?.length;
          const dateMissing = proofRequired && !row.date;
          return <article key={row._id || row.particular || index} className={`min-w-0 overflow-hidden rounded-xl border bg-white ${proofMissing || dateMissing ? 'border-rose-200' : 'border-slate-200'}`}>
            <div className="grid grid-cols-[2.75rem_minmax(0,1fr)_6.5rem_9.5rem] bg-slate-100 text-[10px] font-black uppercase tracking-wider text-slate-500 max-sm:grid-cols-[2.5rem_minmax(0,1fr)]">
            ...
            <div className="grid grid-cols-[2.75rem_minmax(0,1fr)_6.5rem_9.5rem] border-b border-slate-200 text-xs max-sm:grid-cols-[2.5rem_minmax(0,1fr)]">
              ...
              <div className="min-w-0 px-3 py-3"><strong className="break-words text-slate-900">{row.particular}</strong><p className="mt-1 text-[10px] font-bold text-slate-400">Update status, date and supporting proof.</p></div>
              <div className="px-2 py-3 max-sm:col-span-2 max-sm:grid max-sm:grid-cols-[2.5rem_minmax(0,1fr)] max-sm:border-t max-sm:border-slate-200 max-sm:px-0 max-sm:py-0"><span className="hidden bg-slate-50 px-3 py-3 text-[10px] font-black uppercase text-slate-500 max-sm:block">Status</span><div className="max-sm:px-3 max-sm:py-2"><select disabled={!canEdit} value={row.yesNo} onChange={(event) => updateChecklistRow(index, { yesNo: event.target.value }, true)} className={`w-full rounded-lg border bg-white px-2.5 py-2 font-black ${proofMissing || dateMissing ? 'border-rose-300 text-slate-900' : 'border-slate-200'}`}><option value="">Select</option><option>Yes</option><option>No</option></select>{proofMissing && <p className="mt-2 text-[10px] font-black leading-4 text-rose-600">Proof upload is required for a Yes status. Please upload supporting proof.</p>}</div></div>
            ...
            <div className="grid grid-cols-1 gap-3 p-3 md:grid-cols-[minmax(0,1.7fr)_minmax(12rem,1fr)]">
              <section className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-50/60">
                ...
                <div className="p-2"><PurchaseProofDropzone files={row.files || []} required={proofRequired} disabled={!canEdit} busy={busy === `proof-${index}`} onUpload={(selected) => uploadProof(index, selected)} onRemove={(fileIndex, file) => removeProof(index, fileIndex, file)} onPreview={setMailPreview} onError={(text) => setNotice({ type: 'error', text })} /></div>
              </section>
              <section className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-50/60">
